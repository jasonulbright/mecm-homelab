...
# Allow local CM source path as an alternative to download URLs.
# If ConfigurationManagerLocalSource is set, the CM role will use this path instead of downloading.
# This avoids hardcoding version-specific URLs — place extracted CM baseline at this path.
Set-PSFConfig -Module AutomatedLab -Name ConfigurationManagerLocalSource -Value '' -Description 'Local path to extracted CM baseline (e.g., C:\LabSources\SoftwarePackages\CM\ConfigMgr_2509). If set, skips download.' -Validation string
